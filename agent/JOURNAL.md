...
2026-08-19T10:40Z — post-plan — Run 53. Took the standing question about `Browser:Reload` and found the question was the wrong shape, which is the whole run. Run 52 left reload as "the only nav-bar verb with no spoken form" and `STATE.md` said to decide it on `GRAMMAR.md` §5's terms or not at all; deciding it on §5's terms meant not deciding reload at all, because **it is one of forty-six**. §5.1 claimed a command with no spoken form "will be caught by construction, since the action table is the single source of both" — true of the verbs, silent about the browser, and nothing anywhere enumerates the window. So the window was enumerated in a driven build rather than read out of the source, because `#ifdef` and platform-conditional blocks make the file a bad proxy for what a window has: **101 `<key>` elements, 49 distinct commands, three of them this fork's.** Not all forty-six are violations — the fork already has the principle that sorts them, stated twice in `IDEAS.md` for Tab-completion and rail hoisting, that §5 governs *actions* and an affordance running no action needs no word — but the grouping has never been written down, which is why it has never been checked. **The count found a corrupted tree rather than a missing word.** `Browser:Back` is bound to four gestures (`Alt+Left`, `accel+[`, the nav-bar button, the mouse's back button) and none of them were known to pillar B, whose capture rule spawns a child node for every navigation that commits. A traversal commits one, and arrives at `onLocationChange` looking exactly like a click — top-level, to a URL the browser is not at, nothing in the flags to say otherwise — so every press appended a copy of the page being arrived at *underneath* the page being left: `1 com → 2 org → 3 com`, a node deeper per press, a spine nobody browsed, and `up` walking the fiction. Probed in a real window before a line was written, which is run 52's rule applied to its own successor. The sharp part is that this was already understood: `onLocationChange`'s own comment describes the defect precisely, and `#restoring` exists to prevent it — for re-entry, which is the fork's own verb. **It was fixed for the surface this fork replaced and left standing for the one it inherited untouched**, and that is the transferable finding rather than the fix. The signal is `nsIWebProgress.loadType`, which carries the docshell's load command in the parent process; `LOAD_CMD_HISTORY` covers back, forward and any `gotoIndex`, measured in the probe as `4` against `1` for a normal load and `2` for a reload. Three alternatives rejected with reasons: not the URL, because a link back to the page above you is a real visit and sites link upward constantly; not a hook on `Browser:Back` — which is exactly what run 52 did for `Browser:Stop` — because a page calling `history.back()` produces the same traversal and no command event at all, so every site with a "go back" link would still have corrupted the tree, and there is a test that fails if the fix is moved there; and not an `nsISHistoryListener`, which is explicit and is what SessionStore uses but attaches per browsing context and needs re-attaching across process switches, while `loadType` is already on the argument the listener is handed. Landing on the right node needs a per-browser map from session history index to node, and it is exact because re-entry replaces the whole history with a single entry, so every entry above the first was appended by a navigation that also created a node. Method, and it cost more than the code: **six mutations, six caught, and two branches deleted rather than covered.** A pruning pass over the map and a defensive clear on re-entry both survived, and neither could be made to fail by any input — an index is only readable by traversing to it, that needs a history entry, and the navigation which created that entry wrote the map first — so they were deleted, with the invariant written down beside the code. Chasing them raised a case that looked like a real hole (`about:blank` gets no node, so a stale index could answer for it) and a test was written that passed *before* the fix and could never have failed, because `isCapturable` sits above every branch and a traversal onto an uncaptured page returns before the map is consulted; the test was dropped. **A green test that was never red proves only that you did not check.** The other two survivors were the tests' fault rather than the code's and both named reachable cases — a back *after* re-entry, which needs an intervening navigation before there is anything to go back to, and a traversal onto a node that forgetting has deleted while its history entry survives — and were covered, and caught after. Tests: **1015 FOS browser-chrome checks** (up from 990; `browser_zzhistorygesture.js` adds 25), 322 node, 0 failures. Lint clean on every changed file. Docs: `GRAMMAR.md` §5.1 no longer claims an enforcement that does not exist and carries the count instead, `ARCHITECTURE.md` §7 has why a traversal is a move and why the three alternative signals were rejected, and `IDEAS.md` run 53 has the audit, whose actual product — a manifest of every command with a written reason, and a test that fails on any key in neither the grammar nor an exemption — is the next run's item.

2026-08-19T09:40Z — post-plan — Run 54. Built the item STATE had at the top of
the list: the keyset manifest and its test. `keyset-manifest.js` classifies every
command a `<key>` in a running window can reach — nine classes, a written reason
on each — and `browser_zzkeymanifest.js` fails when the window binds a command
the manifest does not name, or the manifest names one the window no longer binds.
That is what turns `GRAMMAR.md` §5.1's "by construction" into a claim about the
window rather than about the action table, which is the sentence run 53 proved
false at the cost of a corrupted trail tree.

**The audit that found that defect had itself undercounted by 42%.** Run 53
reported 101 keys reaching 49 commands, and published it in §5.1. The keys were
right; the commands were not. A key with no `command` attribute still runs
something — a listener on `mainKeyset` keyed by element id, or nothing at all —
and thirty-six of them were invisible to a count that read the attribute: every
devtools key, every sidebar toggle, the nine tab-selection keys, `goHome`, the
six `internal="true"` editing keys. The window binds **101 keys reaching 85
commands, of which two have a word** (`field`, and `stop` from run 52). The
transferable part is one turn past run 53's own: a surface you inherited does not
get audited, and *an audit you published does not get audited either* — not until
something downstream has to be exhaustive. A number in a design document is
...
8 `editing`, 5 `desktop`, 3 `unbound`, 1 `display`, 2 `verb`. The file is
deliberately green with the 26 in it: a ratchet on the count is satisfied by
reclassifying, which is the one move the manifest exists to prevent, and a new
unvoiced command fails the classification check anyway. Two classes are checked
rather than judged — `unbound` (no keystroke on any of its keys, so it cannot
become the shelf awkward bindings get parked on) and `verb` (the named word must
still be in `ACTIONS`).

Three things only writing the reasons found. `Tools:Sanitize` is the sharpest
entry in the file: Clear Recent History is where run 44 wired the context store's
only delete, built on the argument that a record the user cannot remove is not
private for staying on the machine — a capability this fork made itself, given a
dialog and no word. `FOS:CommandBar` is an *affordance*, and is the cleanest
illustration of §5 in the file: opening the bar runs nothing and the spoken path
does not pass through it, so a word for it would be a word for a step only the
keyboard has. And `key_showAllTabs` is bound to nothing at all — it exists so the
View menu can print a shortcut — which only an enumeration obliged to account for
every key would ever ask about.

Research: Rango, the most developed voice-browsing extension there is, ships no
spoken form for back, forward, reload, find, zoom, bookmark, print or save, and
is almost entirely *content* interaction instead. Rejected as an argument about
the principle — §5 already decided that a desktop layer sending a keystroke does
not discharge it — and adopted as an argument about ordering, because of the hole
it exposes: **the manifest enumerates the chrome and there is no hands-free way
to click a link.** `FOSMarks` lists `"link"` among the addressable kinds in its
own doc comment and nothing registers one. A 26-item chrome backlog reads as the
§5 backlog and is not; the backlog's first item is a kind of mark that was
declared and never built. That is now task 1.

Green: 1246 FOS browser-chrome checks across 27 files, 0 failures — the new file
